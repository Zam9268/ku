...
{
	/*短摁处理*/
	if(Key[0].Short_Flag==1)//切换界面
	{Key[0].Short_Flag=0;
		if(++OLED_View_Select>=9) OLED_View_Select = 1;
		OLED_Clear();
	}
	else if(Key[1].Short_Flag==1)//停止电机
	{Key[1].Short_Flag=0;
		MOTOR1_ENABLE_FLAG=0;MOTOR2_ENABLE_FLAG=0;
		
	}
	else if(Key[2].Short_Flag==1)//使能电机
	{Key[2].Short_Flag=0;
		MOTOR1_ENABLE_FLAG=1;MOTOR2_ENABLE_FLAG=1;
